...
int stat_format_output(FILE *infile, FILE *outfile, int format)
{
	...
	while (!csv_read(cf)) {
		...
		RECOGNIZE_TYPE(TREE_CORE);
		RECOGNIZE_TYPE(TREE_ML_CACHE_TOP);
		RECOGNIZE_TYPE(TREE_ML_CACHE_MIDDLE);
		RECOGNIZE_TYPE(TREE_ML_CACHE_BOTTOM);
		RECOGNIZE_TYPE(TREE_CACHE);
