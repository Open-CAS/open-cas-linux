...
static void block_dev_complete_bio_data(struct ocf_io *io, int error)
{
	struct blk_data *data = ocf_io_get_data(io);
	struct bio *bio = data->master_io_req;
...
static void _blockdev_handle_data(ocf_core_t core, struct bio *bio)
{
	...
	ocf_io_set_cmpl(io, NULL, NULL, block_dev_complete_bio_data);
	ocf_io_set_start(io, block_dev_start_bio);

	ocf_core_submit_io(io);
}

static void block_dev_complete_bio_discard(struct ocf_io *io, int error)
{
...
static void block_dev_complete_bio_flush(struct ocf_io *io, int error)
{
	struct bio *bio = io->priv1;
	...
	if (in_interrupt())
		_blockdev_defer_bio(core, bio, _blockdev_handle_data);
	else
...
static void _blkdev_handle_flush(ocf_core_t core, struct bio *bio)
{
	...
	ocf_io_set_cmpl(io, bio, core, block_dev_complete_bio_flush);
...
static void _blockdev_handle_bio(ocf_core_t core, struct bio *bio)
{
	...
	else if (CAS_IS_SET_FLUSH(CAS_BIO_OP_FLAGS(bio)))
