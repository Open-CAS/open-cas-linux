...
#define INVOLUNTARY_PREEMPTION_ERROR \
	"The kernel has been compiled with preemption configurable\n" \
	"at boot time (PREEMPT_DYNAMIC=y). OpenCAS doesn't support\n" \
...
static inline uint32_t involuntary_preemption_enabled(void)
{
	...
	if (config_rt || config_preempt || config_lazy) {
		printk(KERN_ERR OCF_PREFIX_SHORT
				"OpenCAS doesn't support kernels with "
	...
	if (!preempt_model_none() && !preempt_model_voluntary()) {
		printk(KERN_ERR OCF_PREFIX_SHORT "%s\n",
				INVOLUNTARY_PREEMPTION_ERROR);
		return true;
	} else {
		printk(KERN_WARNING OCF_PREFIX_SHORT "%s\n",
				INVOLUNTARY_PREEMPTION_ERROR);
		return false;
	}
#endif
