...
func Json_api() {
	...
	if check_err(decode_header_err) {
...
func json_get_stats(request Params_stats) {
	...
	if check_err(get_stats_err) {
		log_console(get_stats_err.Error())
		return
	}

	stats_pkg_b, marshall_err := json.MarshalIndent(stats_pkg, "", "  ")
	if check_err(marshall_err) {
		log_console("failed marshaling JSON response")
		return
	}
...
func json_list_caches() {
	...
	if check_err(list_caches_err) {
		log_console(list_caches_err.Error())
		return
	}
...
func json_get_cache_info(request Params_cache_info) {
	...
	if check_err(cache_info_err) {
		log_console(cache_info_err.Error())
		return
	}
...
func json_get_core_info(request Params_core_info) {
	...
	if check_err(core_info_err) {
		log_console(core_info_err.Error())
		return
	}
...
func json_get_io_class_info(request Params_io_class_info) {
	...
	if check_err(io_class_info_err) {
		log_console(io_class_info_err.Error())
		return
	}
