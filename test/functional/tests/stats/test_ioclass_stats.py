...
def test_ioclass_stats_basic(random_cls):
    ...
    with TestRun.step(
        "Try retrieving IO class stats for all allowed id values " "and one out of range id"
    ):
            ...
            with TestRun.group(f"Checking{out_of_range} IO class id {class_id}..."):
                ...
                try:
                    ...
                    if expected:
                        ...
                        if not check_string_msg_all(e.output.stderr, get_stats_ioclass_id_not_configured):
                            TestRun.LOGGER.error(
                                f"Wrong message for unused IO class id: {class_id}"
                            )
                    elif not check_string_msg_all(e.output.stderr, get_stats_ioclass_id_out_of_range):
                        TestRun.LOGGER.error(
...
@pytest.mark.parametrize(
    "stat_filter", [StatsFilter.req, StatsFilter.usage, StatsFilter.conf, StatsFilter.blk]
)
...
def test_ioclass_stats_sections(stat_filter, per_core, random_cls):
    ...
    with TestRun.step(
        f"Validate displayed {stat_filter.name} statistics for default IO class for "
        ...
        for cache in caches:
            with TestRun.group(f"Cache {cache.cache_id}"):
                for core in cache.get_core_devices():
                    ...
                    statistics = (
                        core.get_statistics_flat(io_class_id=0, stat_filter=[stat_filter])
                        if per_core
                        else cache.get_statistics_flat(io_class_id=0, stat_filter=[stat_filter])
                    )
    ...
    with TestRun.step(
        f"Validate displayed {stat_filter.name} statistics for every configured IO "
                ...
                for core in cache.get_core_devices():
                    ...
                    for class_id in range(ioclass_config.MAX_IO_CLASS_ID + 1):
                        with TestRun.group(core_info + f"IO class id {class_id}"):
                            statistics = (
                                core.get_statistics_flat(class_id, [stat_filter])
                                if per_core
...
def prepare(random_cls, cache_count=1, cores_per_cache=1):
    ...
    for i, cache_device in enumerate(cache_devices):
        ...
        cache = casadm.start_cache(
            cache_device, force=True, cache_mode=cache_modes[i], cache_line_size=random_cls
        )
        caches.append(cache)
        ...
        for core_device in core_devices[i * cores_per_cache : (i + 1) * cores_per_cache]:
            TestRun.LOGGER.info(f"Adding core device {core_device.path} to cache {cache.cache_id}")
