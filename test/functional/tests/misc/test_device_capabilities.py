...
def test_device_capabilities():
    ...
    max_io_size_path = os.path.join(
        disk_utils.get_sysfs_path(core_device.get_device_id()), "queue/max_sectors_kb"
    )
    ...
    iteration_settings = [
        ...
        {"device": "standard core device", "max_sectors_kb": int(default_max_io_size)},
    ...
    for i in range(0, len(iteration_settings)):
        ...
        if device == "SCSI-debug module":
            group_title += (
                f"dev_size_mb = {iteration_settings[i]['dev_size_mb']} | "
        ...
        with TestRun.group(group_title):
            ...
            with TestRun.step(
                "Compare capabilities for CAS device, cache and core "
...
def prepare_cas_device(cache_device, core_device):
    ...
    try:
        ...
        TestRun.LOGGER.info(
            "Cannot add core device with mismatching logical sector size. "
...
def measure_capabilities(dev):
    dev_capabilities = {}
    dev_id = (
        dev.parent_device.get_device_id() if isinstance(dev, Partition) else dev.get_device_id()
    )
...
def compare_capabilities(cache_device, core_device, cache, core, msg):
    if core is None:
        ...
        core_dev_sectors_num = disk_utils.get_size(
            core_device.get_device_id()
        ) / disk_utils.get_block_size(core_device.get_device_id())
