...
def test_random_operations():
    ...
    pass_criteria:
      - No system crash.
    """
    test_runtime = timedelta(minutes=10)

    with TestRun.step("Prepare disks"):
        cache_dev = TestRun.disks["cache"]
        core_dev = TestRun.disks["core"]

        cache_dev.create_partitions([Size(1, Unit.GibiByte)] * 10)
    ...
    with TestRun.step("Run random operations"):
        ...
        try:
            ...
            step_name_list.append(step_name.__name__)
            step_list = "\n".join(step_name_list)
...
def add_core(
    ...
    return (
        f"Device {random_core_device.path} added as core to " f"cache {cache_to_add_core.cache_id}"
    )
