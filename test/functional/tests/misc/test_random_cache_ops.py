...
from api.cas.casadm_parser import get_caches, get_inactive_cores
...
def test_random_operations():
    """
    title: Stress test for CAS allowed operations.
    description: |
      Check that CAS can handle all allowed operation in random run order.
    pass_criteria:
      - No system crash.
    """
    test_runtime = timedelta(minutes=30)

    with TestRun.step("Prepare disks"):
        cache_dev = TestRun.disks["cache"]
        core_dev = TestRun.disks["core"]

        cache_dev.create_partitions([Size(1, Unit.GibiByte)] * 20)
        core_dev.create_partitions([Size(2, Unit.GibiByte)] * 20)
...
def start_cache(devices: dict) -> str | None:
    available_cache_devices = devices.get("available_cache_devices")
    used_cache_devices = devices.get("used_cache_devices")
    available_core_devices = devices.get("available_core_devices")
        ...
        if check_stderr_msg(output.output, start_cache_with_existing_metadata):
            if random.choice([True, False]):
                try:
                    ...
                    active_cores = loaded_cache.get_core_devices()
...
def stop_cache(devices: dict) -> str | None:
    available_cache_devices = devices.get("available_cache_devices")
    used_cache_devices = devices.get("used_cache_devices")
    available_core_devices = devices.get("available_core_devices")
    used_core_devices = devices.get("used_core_devices")
    ...
    active_cores = cache_to_stop.get_core_devices()
    ...
    if random.choice([True, False]):
        ...
        cache_to_stop.stop(no_data_flush=True)
        for device in used_cache_devices:
            if device.path == cache_to_stop.cache_device.path:
                available_cache_devices.append(device)
                used_cache_devices.remove(device)
...
def set_param(devices: dict) -> str | None:
    cache_list = get_caches()

    if not cache_list:
        return

    cache_to_set_param = random.choice(cache_list)
    set_param_list = [
    ...
    match param_to_set:
        case cache_to_set_param.set_seq_cutoff_parameters:
            if random.choice([True, False]):
                ...
                core_list = cache_to_set_param.get_core_devices()
                if not core_list:
                    return
...
def set_cache_mode(devices: dict):
    cache_list = get_caches()
    if not cache_list:
        return
...
def add_core(devices: dict) -> str | None:
    available_core_devices = devices.get("available_core_devices")
    used_core_devices = devices.get("used_core_devices")
...
def remove_core(devices: dict):
    ...
    cache_list = [cache for cache in cache_list if cache.get_core_devices()]
    random_cache = random.choice(cache_list)
    core_list = random_cache.get_core_devices()
    random_core = random.choice(core_list)
...
def remove_inactive(devices: dict):
    available_core_devices = devices.get("available_core_devices")
    cache_list = get_caches()
...
def reset_counters(devices: dict):
    cache_list = get_caches()
...
def zero_metadata(devices: dict):
    available_cache_devices = devices.get("available_cache_devices")
...
def run_fio(devices: dict) -> str | None:
    cache_list = get_caches()
    if not cache_list:
        return
