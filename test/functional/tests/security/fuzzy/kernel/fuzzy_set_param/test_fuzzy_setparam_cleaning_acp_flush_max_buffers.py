...
def test_fuzzy_setparam_cleaning_acp_flush_max_buffers(
    cache_line_size, unaligned_io, use_io_scheduler
):
    """
    title: test_fuzzy_setparam_cleaning_acp_flush_max_buffers
    ...
    with TestRun.step("using Peach Fuzzer to create 'get parameter' command and run it."):
        valid_values = [str(x).encode("ascii") for x in range(1, 10001)]
