...
def test_fuzzy_setparam_cleaning_alru_activity_threshold(
    cache_line_size, unaligned_io, use_io_scheduler
):
    """
    title: test_fuzzy_setparam_cleaning_alru_activity_threshold
    description: Fuzzy test for casadm 'set parameter' command – wake up in setting activity_threshold parameters,
    Using Peach Fuzzer check Open CAS ability of handling wrong wake up time in 'set parameter' command.
    pass_criteria:
    ...
    for index, cmd in TestRun.iteration(
        ...
        with TestRun.step(f"Iteration {index + 1}"):
            run_cmd_and_validate(
                ...
                "Output_format",
                valid_values,
                lambda param: str(int(param)).encode("ascii") if param.isdigit() else param,
            )
