...
def test_fuzzy_setparam_cleaning_acp_wakeup(cache_line_size, unaligned_io, use_io_scheduler):
    """
    title: test_fuzzy_setparam_cleaning_acp_wakeup
    description: Fuzzy test for casadm 'set parameter' command – wake up in setting cleaning-acp parameters,
    ...
    with TestRun.step("using Peach Fuzzer to create 'get parameter' command and run it."):
        valid_values = [str(x).encode("ascii") for x in range(0, 10001)]
        PeachFuzzer.generate_config(get_fuzz_config("uint.yml"))
        base_cmd = set_param_cleaning_acp_cmd(cache_id=cache.cache_id, wake_up="{param}").encode(
            "ascii"
        )
