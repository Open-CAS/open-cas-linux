...
def test_fuzzy_getparam_seqcutoff_core_id(cache_line_size, unaligned_io, use_io_scheduler):
    """
    title: test_fuzzy_getparam_seqcutoff_core_id
    description:  Fuzzy test for casadm 'get parameter' command – core id in getting sequential cutoff parameters.
    Using Peach Fuzzer check OpenCAS ability of handling wrong core id in 'get parameter' command
    pass_criteria:
    ...
    with TestRun.step("Using Peach Fuzzer to create 'get parameter' command and run it."):
        ...
        base_cmd = get_param_cutoff_cmd(str(core.cache_id), core_id="{param}").encode("ascii")
