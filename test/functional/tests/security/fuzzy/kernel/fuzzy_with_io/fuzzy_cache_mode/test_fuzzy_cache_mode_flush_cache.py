...
def test_fuzzy_cache_mode_flush_cache(cache_mode, cache_line_size, cleaning_policy, unaligned_io,
                                      use_io_scheduler):
    """
        title: Fuzzy test for casadm 'set cache mode' command - flush cache.
        description: Using Peach Fuzzer check Open CAS ability of handling wrong flush cache option
    ...
    with TestRun.step("Prepare PeachFuzzer"):
        ...
        for mode in modes:
            base_cmd = set_cache_mode_cmd(mode, str(cache.cache_id), "{param}").encode('ascii')
            generator = PeachFuzzer.get_fuzzed_command(base_cmd, int(TestRun.usr.fuzzy_iter_count /
                                                                     len(modes)))
    ...
    for index, cmd in TestRun.iteration(enumerate(commands),
        ...
        with TestRun.step(f"Iteration {index + 1}"):
            run_cmd_and_validate(cmd, "Flush cache option", cmd.param in valid_values)
