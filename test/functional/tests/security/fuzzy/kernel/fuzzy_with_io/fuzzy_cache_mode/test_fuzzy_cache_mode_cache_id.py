...
def test_fuzzy_cache_mode_cache_id(cache_mode, cache_line_size, cleaning_policy, unaligned_io,
                                   use_io_scheduler):
    """
    ...
    with TestRun.step("Prepare PeachFuzzer"):
        ...
        for mode in modes:
            base_cmd = set_cache_mode_cmd(mode, "{param}", "no").encode('ascii')
            generator = PeachFuzzer.get_fuzzed_command(base_cmd, int(TestRun.usr.fuzzy_iter_count / len(modes)))
