...
def test_fuzzy_io_class_load_config_file(
    ...
    description: |
        Using Peach Fuzzer check Open CAS ability of handling wrong file name in
    ...
    with TestRun.step("Prepare PeachFuzzer"):
        ...
        base_cmd = load_io_classes_cmd(cache_id=str(core.cache_id), file="{param}")
    ...
    for index, cmd in TestRun.iteration(
        ...
        with TestRun.step(f"Iteration {index + 1}"):
            ...
            run_cmd_and_validate(
                cmd=cmd,
                value_name="Io class config file path",
