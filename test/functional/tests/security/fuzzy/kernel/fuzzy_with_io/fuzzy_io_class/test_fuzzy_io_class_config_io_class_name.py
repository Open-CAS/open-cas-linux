...
from tests.security.fuzzy.kernel.common.common import get_cmd, run_cmd_and_validate, \
    prepare_cas_instance, get_fuzz_config
...
parametrized_keywords = [
    "directory", "file_name_prefix", "extension", "process_name",  # string - based
    "file_size", "io_class", "lba", "pid", "file_offset", "request_size"  # number - based
]
parameterless_keywords = [
    "metadata", "direct", "done"
]
...
def test_fuzzy_io_class_config_io_class_name(cache_mode, cache_line_size, cleaning_policy,
                                             unaligned_io, use_io_scheduler):
    """
        title: Fuzzy test for IO class configuration content – IO class name.
        description: Using Peach Fuzzer check Open CAS ability of handling wrong IO class name in
    ...
    with TestRun.step("Run fio in background"):
        ...
        io_class = IoClass(1, f"file_size:{Operator.le.name}:97517568", 255)
    ...
    for index, parameter in TestRun.iteration(enumerate(parameters),
        ...
        with TestRun.step(f"Iteration {index + 1}"):
            ...
            run_cmd_and_validate(cmd, "Io Class name", __is_valid_io_class_name(parameter))
...
def __get_fuzzed_parameters(parameters):
    ...
    for index, param in enumerate(parameters):
        if index < TestRun.usr.fuzzy_iter_count // 10:
            ...
            short_names = [param for param in fuzzed_parameters if
                           len(param) < max_length_of_single_condition]
...
def __is_valid_io_class_name(value: str):
...
def __validate_single_condition(value: str):
    ...
    if condition_key in ["directory", "file_name_prefix", "extension", "process_name"]:
        if 0 < len(condition_value) <= 255:
            return True
    elif condition_key in ["file_size", "io_class", "lba", "pid", "file_offset", "request_size"]:
        #  key:number case
