...
def test_fuzzy_io_class_config_io_class_id(cache_mode, cache_line_size, cleaning_policy,
                                           unaligned_io, use_io_scheduler):
    """
        title: Fuzzy test for IO class configuration content – IO class id.
        description: Using Peach Fuzzer check Open CAS ability of handling wrong IO class id in
    ...
    with TestRun.step("Run fio in background"):
        ...
        io_class = IoClass(1, f"file_size:{Operator.le.name}:97517568", 255, "0.00")
    ...
    for index, parameter in TestRun.iteration(enumerate(parameters),
        ...
        with TestRun.step(f"Iteration {index + 1}"):
            ...
            run_cmd_and_validate(cmd, "Io class id", __is_valid_class_id(parameter))

    with TestRun.step("Stop 'fio'"):
        TestRun.executor.kill_process(fio_pid)


def __is_valid_class_id(parameter):
    # io class with id 0 already exists so minimum is id 1
    return parameter.isdigit() and 1 <= int(parameter) <= 32
