...
def test_fuzzy_io_class_config_priority(cache_mode, cache_line_size, cleaning_policy, unaligned_io,
                                        use_io_scheduler):
    """
        title: Fuzzy test for IO class configuration content – IO class priority.
        description: Using Peach Fuzzer check Open CAS ability of handling wrong IO class priority
    ...
    for index, parameter in TestRun.iteration(enumerate(parameters),
        ...
        with TestRun.step(f"Iteration {index + 1}"):
            ...
            run_cmd_and_validate(cmd, "Priority", __is_value_valid(parameter))

    with TestRun.step("Stop 'fio'"):
        TestRun.executor.kill_process(fio_pid)


def __is_value_valid(parameter):
    if parameter == '':
        return True
    elif parameter.isdigit() and 1 <= int(parameter) <= 16384:
        return True
