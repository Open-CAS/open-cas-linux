...
from api.cas.cli import get_param_seq_detect_cmd
...
def test_fuzzy_get_param_seq_detect_output_format(
    cache_mode, cache_line_size, unaligned_io, use_io_scheduler
):
    """
    title: Fuzzy test for casadm 'get parameter' command for sequence detector – output format.
    description: |
        Using Peach Fuzzer check Open CAS ability of handling wrong output format
        in 'get parameter' for sequence detector command.
    pass_criteria:
    ...
    with TestRun.step("Using Peach Fuzzer to create 'get parameter' command and run it."):
        ...
        base_cmd = get_param_seq_detect_cmd(
