...
def test_fuzzy_get_param_prefetch_cache_id(
    cache_mode, cache_line_size, unaligned_io, use_io_scheduler
):
    """
    title: Fuzzy test for casadm 'get parameter' command for prefetch policy – cache id.
    description: |
        Using Peach Fuzzer check Open CAS ability of handling wrong cache id in 'get parameter'
        command for prefetch policy parameters.
    pass_criteria:
    ...
    with TestRun.step("Using Peach Fuzzer create 'get parameter' command and run it."):
        ...
        base_cmd = get_param_prefetch_cmd(cache_id="{param}")
