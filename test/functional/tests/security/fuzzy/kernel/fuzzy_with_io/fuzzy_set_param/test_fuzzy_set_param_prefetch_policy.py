...
from api.cas.cli import set_param_prefetch_cmd
...
def test_fuzzy_set_param_prefetch_policy(
    cache_mode, cache_line_size, unaligned_io, use_io_scheduler
):
    """
    title: Fuzzy test for casadm 'set parameter' command for prefetch – policy.
    description: |
        Using Peach Fuzzer check Open CAS ability of handling wrong prefetch policy in
        'set parameter' command for prefetch parameters.
    pass_criteria:
    ...
    with TestRun.step("Using Peach Fuzzer create 'set parameter' command and run it."):
        valid_values = [str(PrefetchPolicy.none).encode("ascii")]
        valid_values += [
            str(policy).encode("ascii") for policy in PrefetchPolicy if policy.value
        ]
        PeachFuzzer.generate_config(get_fuzz_config("prefetch_policy.yml"))
        base_cmd = set_param_prefetch_cmd(cache_id=str(core.cache_id), policy="{param}")
