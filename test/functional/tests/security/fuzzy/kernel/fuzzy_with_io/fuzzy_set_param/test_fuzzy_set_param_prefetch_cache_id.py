...
def test_fuzzy_set_param_prefetch_cache_id(
    cache_mode, cache_line_size, unaligned_io, use_io_scheduler
):
    """
    title: Fuzzy test for casadm 'set parameter' command for prefetch – cache id.
    description: |
        Using Peach Fuzzer check Open CAS ability of handling wrong cache id in 'set parameter'
        command for prefetch parameters.
    pass_criteria:
    ...
    with TestRun.step("Using Peach Fuzzer create 'set parameter' command and run it."):
        ...
        base_cmd = set_param_prefetch_cmd(
            cache_id="{param}",
            policy=str(PrefetchPolicy.DEFAULT),
        )
