...
dirty_ratio_threshold_min, dirty_ratio_threshold_max = FlushParametersAlru.alru_params_range().dirty_ratio_threshold
...
def test_fuzzy_set_param_cleaning_alru_dirty_ratio_threshold(
    cache_mode, cache_line_size, unaligned_io, use_io_scheduler
):
    """
    title: Fuzzy test for casadm 'set parameter' command for alru cleaning – dirty ratio threshold.
    description: |
        Using Peach Fuzzer check Open CAS ability of handling wrong dirty ratio threshold value in
    ...
    for index, cmd in TestRun.iteration(
        ...
        with TestRun.step(f"Iteration {index + 1}"):
            ...
            run_cmd_and_validate(
                cmd=cmd,
                value_name="Dirty ratio trigger threshold",
...
def __is_valid(parameter):
    try:
        ...
        return False
    return dirty_ratio_threshold_min <= value <= dirty_ratio_threshold_max
