...
from api.cas.cli import set_param_prefetch_readahead_cmd
...
def test_fuzzy_set_param_prefetch_readahead_cache_id(
    cache_mode, cache_line_size, unaligned_io, use_io_scheduler
):
    """
    title: Fuzzy test for casadm 'set parameter' command for prefetch readahead – cache id.
    description: |
        Using Peach Fuzzer check Open CAS ability of handling wrong cache id in 'set parameter'
        command for prefetch readahead parameters.
    pass_criteria:
    ...
    with TestRun.step("Using Peach Fuzzer create 'set parameter' command and run it."):
        ...
        default_threshold = PrefetchParametersReadahead.default_readahead_params().threshold
        PeachFuzzer.generate_config(get_fuzz_config("cache_id.yml"))
        base_cmd = set_param_prefetch_readahead_cmd(
            cache_id="{param}",
            threshold=str(int(default_threshold.get_value(Unit.KibiByte))),
        )
