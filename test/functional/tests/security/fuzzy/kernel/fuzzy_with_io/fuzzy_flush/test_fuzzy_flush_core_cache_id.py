...
def test_fuzzy_flush_core_cache_id(cache_mode, cache_line_size, cleaning_policy, unaligned_io,
                                   use_io_scheduler):
    """
        title: Fuzzy test for casadm 'flush core' command - cache id.
    ...
    with TestRun.step("Prepare PeachFuzzer"):
        ...
        base_cmd = flush_core_cmd("{param}", core.core_id).encode('ascii')
    ...
    for index, cmd in TestRun.iteration(enumerate(commands),
        ...
        with TestRun.step(f"Iteration {index + 1}"):
            run_cmd_and_validate(cmd, "Cache id", cmd.param in valid_values)
