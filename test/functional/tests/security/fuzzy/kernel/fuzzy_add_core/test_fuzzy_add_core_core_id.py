...
def test_fuzzy_add_core_core_id(cache_mode, cache_line_size, cleaning_policy, unaligned_io,
                                use_io_scheduler):
    """
        title: Fuzzy test for casadm 'add core' command – core id
        description: Using Peach Fuzzer check Open CAS ability of handling wrong core id in
            'add core' command.
        pass_criteria:
    ...
    with TestRun.step("Prepare PeachFuzzer"):
        ...
        base_cmd = add_core_cmd(str(cache.cache_id), core_disk.path, "{param}").encode('ascii')
    ...
    for index, cmd in TestRun.iteration(enumerate(commands),
        ...
        with TestRun.step(f"Iteration {index + 1}"):
            output = run_cmd_and_validate(cmd, "Core id",
                                          __is_valid_core_id(cmd.param.decode('ascii', 'ignore')))
...
def __is_valid_core_id(parameter):
    return parameter.isdigit() and 0 <= int(parameter) <= 4095
