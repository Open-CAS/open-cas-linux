...
def test_fuzzy_remove_core_core_id(cache_mode, cache_line_size, cleaning_policy, unaligned_io,
                                   use_io_scheduler):
    """
        title: Fuzzy test for casadm 'remove core' command - core id.
    ...
    with TestRun.step("Prepare PeachFuzzer"):
        ...
        base_cmd = remove_core_cmd(core.cache_id, "{param}").encode('ascii')
    ...
    for index, cmd in TestRun.iteration(enumerate(commands),
        ...
        with TestRun.step(f"Iteration {index + 1}"):
            output = run_cmd_and_validate(cmd, "Core id", cmd.param in valid_values)
