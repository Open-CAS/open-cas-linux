...
def test_fuzzy_stop_cache(cache_mode, cache_line_size, cleaning_policy, unaligned_io,
                          use_io_scheduler):
    """
        title: Fuzzy test for casadm 'stop cache' command.
    ...
    with TestRun.step("Prepare PeachFuzzer"):
        ...
        base_cmd = stop_cmd("{param}").encode('ascii')
        ...
        with TestRun.step(f"Iteration {index + 1}"):
            ...
            if output.exit_code == 0:
                with TestRun.step("Restart Open CAS"):
                    cache, core = prepare_cas_instance(cache_disk.partitions[0], core_disk,
                                                       cache_mode, cache_line_size,
                                                       KernelParameters(unaligned_io,
                                                                        use_io_scheduler),
                                                       cleaning_policy, create_partition=False)
