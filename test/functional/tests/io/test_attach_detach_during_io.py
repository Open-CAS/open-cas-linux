...
def test_attach_detach_during_io(cache_mode):
    ...
    with TestRun.step("Prepare cache and core devices"):
        ...
        cache_dev.create_partitions([
            # cache_part1:
            Size(40, Unit.MebiByte),
            # cache_parts:
            Size(60, Unit.MebiByte),
            Size(100, Unit.MebiByte),
            Size(50, Unit.MebiByte),
            Size(80, Unit.MebiByte)
        ])
        cache_part1 = cache_dev.partitions[0]
        cache_parts = cache_dev.partitions[1:]
    ...
    with TestRun.step("Start cache and add core"):
        cache = start_cache(cache_part1, force=True)
    ...
    with TestRun.step("Run random mixed read and write workload"):
        fio = (
            ...
            .time_based()
            .target(core.path)
            .blocksize_range([(Size(512, Unit.Byte), Size(32, Unit.MegaByte))])
        )
    ...
    with TestRun.step("Randomly detach and attach cache during I/O"):
        ...
        while TestRun.executor.check_if_process_exists(fio_pid):
            ...
            cache_to_attach = random.choice(cache_parts)
