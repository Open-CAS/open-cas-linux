...
@pytest.mark.os_dependent
@pytest.mark.remote_only
@pytest.mark.require_incompatible_kernel
@pytest.mark.require_disk("cache", DiskTypeSet([DiskType.optane, DiskType.nand]))
@pytest.mark.require_disk("core", DiskTypeLowerThan("cache"))
def test_cas_startup_incompatible_kernel():
    """
    title: Test for starting CAS on system startup after reboot to incompatible kernel.
    pass_criteria:
      - System does not crash.
      - CAS modules are loaded before partitions are mounted.
      - Cache is loaded before partitions are mounted.
    ...
    with TestRun.step("Change kernel version."):
        previous_kernel = get_current_kernel_version()
        TestRun.LOGGER.info(f"Current kernel version: {previous_kernel}")
        switch_kernel(compatible=False)
