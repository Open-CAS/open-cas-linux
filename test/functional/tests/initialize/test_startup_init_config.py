...
def test_lazy_startup_core_path_by_id(cache_mode, reboot_type):
    """
    title: Startup when cores are set in config with wrong by-id path.
    description: |
        Test checking initializing cache with different links to core devices in startup config
    pass_criteria:
      - System does not crash
...
def test_lazy_startup_core_path_not_by_id(cache_mode, reboot_type):
    """
    title: Startup when cores are set in config with short path.
    description: |
        Test checking initializing cache with short paths (/dev/sdx) of core devices
        in startup config
    pass_criteria:
      - System does not crash
