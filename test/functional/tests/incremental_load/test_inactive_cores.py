...
def test_core_inactive_stats_conf():
    ...
    with TestRun.step(
        ...
        if stats.config_stats.core_dev != 3:
            TestRun.fail("Wrong number of cores after loading the cache")
        if stats.config_stats.inactive_core_devices != 1:
            TestRun.fail("Wrong number of inactive cores after loading the cache")
...
def test_core_inactive_stats_usage():
    ...
    with TestRun.step("Check cores statistics"):
        ...
        if cache_usage_stats.inactive_occupancy != inactive_cores_occupancy_stats:
            TestRun.LOGGER.error("Wrong number of occupancy blocks in inactive cache usage stats")
        if cache_usage_stats.inactive_dirty != inactive_cores_dirty_stats:
            TestRun.LOGGER.error("Wrong number of dirty blocks in cache inactive usage stats")
        if cache_usage_stats.inactive_clean != inactive_cores_clean_stats:
            TestRun.LOGGER.error("Wrong number of clean blocks in cache inactive usage stats")
        ...
        if inactive_occupancy_perc != cache_usage_stats_percentage.inactive_occupancy:
            TestRun.LOGGER.error("Wrong occupancy percentage in usage stats")
        if inactive_dirty_perc != cache_usage_stats_percentage.inactive_dirty:
            TestRun.LOGGER.error("Wrong dirty blocks percentage in usage stats")
        if inactive_clean_perc != cache_usage_stats_percentage.inactive_clean:
            TestRun.LOGGER.error("Wrong clean blocks percentage in usage stats")
