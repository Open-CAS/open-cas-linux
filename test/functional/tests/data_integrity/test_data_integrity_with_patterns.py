...
@pytest.mark.require_disk("cache", DiskTypeSet([DiskType.optane, DiskType.nand]))
@pytest.mark.require_disk("core", DiskTypeLowerThan("cache"))
def test_data_integrity_hi_lo_pattern(cache_mode):
    """
        title:
          Data integrity test with high-low write pattern with 2 hours duration time
        description: |
          Run fio with high-low pattern (twice - first time high, second time low) on each core
          and verify if there are no write errors.
        pass_criteria:
            - System does not crash.
            - Fio does not return any errors.
    """
    ...
    with TestRun.step(f"Run fio with high-low pattern."):
        fio_high = prepare_unified_fio(fio_targets, Pattern.high)
        fio_high_output = fio_high.run()
        fio_low = prepare_unified_fio(fio_targets, Pattern.low)
        fio_low_output = fio_low.run()

    with TestRun.step("Check for fio errors."):
        fio_errors = fio_high_output[0].total_errors()
        if fio_errors:
            TestRun.LOGGER.error(f"Found {fio_errors} errors in fio output")

        fio_errors = fio_low_output[0].total_errors()
        if fio_errors:
            TestRun.LOGGER.error(f"Found {fio_errors} errors in fio output")
